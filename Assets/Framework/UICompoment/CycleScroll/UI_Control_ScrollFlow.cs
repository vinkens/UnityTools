...
public class UI_Control_ScrollFlow : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerClickHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        ...
        if (add_vect.sqrMagnitude <= 1)
        {
            ...
            UI_Control_ScrollFlow_Item script = eventData.pointerPressRaycast.gameObject.GetComponentInParent<UI_Control_ScrollFlow_Item>();
            if (script != null)
            {
                AnimToItem(script);
            }
    ...
    public void AnimToItem(UI_Control_ScrollFlow_Item item)
    {
        float k = item.v;
        k = 1.1f - k;
        if (k > 0)
    ...
    {
        float normalizedPos = PositionCurve.Evaluate((v - 0.1f) * 0.5f);
        float pos = normalizedPos * Width - Width / 2;
        return pos;
    }
